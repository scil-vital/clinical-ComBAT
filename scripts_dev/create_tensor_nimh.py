...
def _build_arg_parser():
    ...
    p.add_argument("reference_path", help="Path to reference images")
...
def main():
    ...
    reference_image = join(args.reference_path)
    ...
    for sid in inputs:
        if len(sid) > 2:
            print(sid)
            loadings.append(
                process_files(args, sid, brain_mask, bundle_names, bundles_brain_voxels)
            )
